...
class ViewUndergradDetails(tk.Frame):
    """ Add Popup Window """
    ...
    def _create_widgets(self):
        ...
        self._supervisor = tk.Label(self, text="Minor: ").grid(row=8, column=0)
        self._supervisor_value = tk.Label(self, text=self._student['minor']).grid(row=8, column=1)
        self._undergrad_degree = tk.Label(self, text="Minimum credit: ").grid(row=9, column=0)
        self._undergraddegree_value = tk.Label(self, text=self._student['minimum_credit']).grid(row=9, column=1)
